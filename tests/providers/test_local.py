...
from pidgin.providers.local import LocalProvider, LOCAL_MODELS
...
class TestLocalProvider:
    ...
    async def test_stream_response_invalid_model(self, mock_messages):
        ...
        assert len(chunks) == 1
        assert chunks[0] == "Error: LocalProvider only supports 'test' model. Got: invalid"
    ...
    async def test_model_import_error_handling(self, mock_messages):
        ...
        # Mock import error
        with patch('pidgin.providers.test_model.LocalTestModel', side_effect=ImportError("Module not found")):
