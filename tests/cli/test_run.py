...
def mock_dependencies():
    ...
    with patch('pidgin.cli.run._run_conversations') as mock_run, \
         patch('pidgin.cli.run._prompt_for_model') as mock_prompt, \
         patch('pidgin.cli.run.generate_experiment_name') as mock_gen_name, \
         patch('pidgin.cli.run.validate_model_id') as mock_validate, \
         patch('pidgin.cli.run.check_ollama_available') as mock_ollama, \
         patch('pidgin.cli.run.parse_dimensions') as mock_parse_dims:
...
class TestRunCommandValidation:
    ...
    def test_turns_validation_maximum(self, cli_runner):
        ...
        assert result.exit_code != 0
        assert "--turns" in result.output and "1001 is not in the range" in result.output
    ...
    def test_repetitions_validation_minimum(self, cli_runner):
        ...
        assert result.exit_code != 0
        assert "--repetitions" in result.output and "is not in the range" in result.output
    ...
    def test_temperature_validation_minimum(self, cli_runner):
        ...
        assert result.exit_code != 0
        assert "--temperature" in result.output and "is not in the range" in result.output
    ...
    def test_convergence_profile_validation(self, cli_runner):
        ...
        assert result.exit_code != 0
        assert "--convergence-profile" in result.output and "is not one of" in result.output
    ...
    def test_convergence_action_validation(self, cli_runner):
        ...
        assert result.exit_code != 0
        assert "--convergence-action" in result.output and "is not one of" in result.output
...
class TestRunCommandConfiguration:
    ...
    def test_custom_output_directory(self, cli_runner, mock_dependencies, temp_output_dir):
...
class TestRunCommandErrorHandling:
    ...
    def test_api_key_error_handling(self, cli_runner, mock_dependencies):
        ...
        mock_dependencies['validate_model_id'].side_effect = APIKeyError("Test API key error")
