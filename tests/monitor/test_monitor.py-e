...
def sample_experiment_dir(temp_experiments_dir):
    ...
    events = [
        ...
        {"event_type": "MessageSentEvent", "timestamp": now.isoformat()},
    ]
...
def monitor(temp_experiments_dir):
    ...
    with patch(
        "pidgin.monitor.monitor.get_experiments_dir", return_value=temp_experiments_dir
...
class TestMonitor:
    ...
    def test_get_recent_errors_with_sample_data(self, monitor, sample_experiment_dir):
        ...
        error_events = [
            e for e in result if e.get("event_type") in ["APIErrorEvent", "ErrorEvent"]
        ]
    ...
    def test_get_failed_conversations_no_experiments(
        self, monitor, temp_experiments_dir
    ):
        """Test getting failed conversations when no experiments exist."""
        result = monitor.get_failed_conversations()
        assert result == []

    def test_get_failed_conversations_with_sample_data(
        self, monitor, sample_experiment_dir
    ):
    ...
    def test_get_experiment_states_calls_state_builder(self, monitor):
        ...
        monitor.state_builder.list_experiments.assert_called_once_with(
            monitor.exp_base, status_filter=None
        )
    ...
    def test_build_display_integration(self, monitor):
        ...
        with patch.object(
            monitor, "get_experiment_states", return_value=[]
        ), patch.object(monitor, "build_header", return_value=Mock()), patch.object(
            monitor, "build_errors_panel", return_value=Mock()
        ), patch.object(
            monitor, "build_experiments_panel", return_value=Mock()
        ), patch.object(
            monitor, "build_conversations_panel", return_value=Mock()
        ):
