...
class TestConversationFlow:
    ...
    async def test_basic_conversation_with_test_model(self, tmp_path):
        ...
        conversation = Conversation(
            agents=agents, initial_prompt="Let's test the conversation system"
        )
    ...
    async def test_event_serialization(self, tmp_path):
        ...
        try:
            ...
            from pidgin.core.events import MessageCompleteEvent, TurnStartEvent

            conversation_id = "test_conv_456"

            # Turn started
            await bus.emit(
                TurnStartEvent(conversation_id=conversation_id, turn_number=1)
            )
            ...
            test_message = Message(
                role="assistant", content="Test response", agent_id="agent_a"
            )
            await bus.emit(
