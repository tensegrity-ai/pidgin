...
from pidgin.core.events import MessageCompleteEvent, TokenUsageEvent
...
class TestTokenUsageHandler:
    ...
    def handler(self, mock_storage, mock_tracker):
        ...
        with patch(
            "pidgin.database.token_handler.get_token_tracker", return_value=mock_tracker
        ):
    ...
    def test_handle_token_usage_with_breakdown(
        self, handler, mock_storage, mock_tracker
    ):
        ...
        assert expected_rpm in [
            25,
            50,
            100,
        ], f"Expected rpm to be 25, 50, or 100, got {expected_rpm}"
    ...
    def test_handle_token_usage_without_breakdown(
        self, handler, mock_storage, mock_tracker
    ):
    ...
    def test_handle_token_usage_unknown_model(
        self, handler, mock_storage, mock_tracker
    ):
    ...
    def test_calculate_costs_various_providers(self, handler):
        ...
        costs = handler._calculate_costs(
            "anthropic", "claude-3-5-sonnet-20241022", 1000, 300, 700
        )
        ...
        # Test OpenAI pricing
        costs = handler._calculate_costs("openai", "gpt-4o-mini", 1000, 400, 600)
        ...
        # Test Google pricing
        costs = handler._calculate_costs("google", "gemini-1.5-flash", 1000, 200, 800)
    ...
    def test_calculate_costs_partial_model_match(self, handler):
        ...
        costs = handler._calculate_costs(
            "anthropic", "claude-3-5-sonnet-20241022-preview", 1000, 500, 500
        )
