...
class TestTurnExecutor:
    ...
    def mock_config(self):
        ...
        config = Mock()
        config.get_convergence_config = Mock(
            return_value={"convergence_threshold": 0.85, "convergence_action": "stop"}
        )
        return config

    @pytest.fixture
    def executor(
        self, mock_bus, mock_message_handler, mock_convergence_calculator, mock_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_single_turn_success(
        self, executor, mock_bus, mock_message_handler, mock_convergence_calculator
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_single_turn_interrupted_agent_a(
        self, executor, mock_bus, mock_message_handler
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_single_turn_interrupted_agent_b(
        self, executor, mock_bus, mock_message_handler
    ):
    ...
    @pytest.mark.asyncio
    async def test_high_convergence_stop_action(
        self, executor, mock_bus, mock_message_handler, mock_convergence_calculator
    ):
    ...
    @pytest.mark.asyncio
    async def test_high_convergence_warn_action(
        self,
        executor,
        mock_bus,
        mock_message_handler,
    ...
    @pytest.mark.asyncio
    async def test_convergence_threshold_override(
        self, executor, mock_bus, mock_message_handler, mock_convergence_calculator
    ):
    ...
    @pytest.mark.asyncio
    async def test_convergence_action_override(
        self, executor, mock_bus, mock_message_handler, mock_convergence_calculator
    ):
    ...
    @pytest.mark.asyncio
    async def test_message_handler_gets_correct_params(
        self, executor, mock_message_handler
    ):
        ...
        async def capture_and_return(conv_id, agent, turn_num, messages, handler):
            ...
            if agent.id == "agent_a":
                return make_message(
                    content="New A", agent_id="agent_a", role="assistant"
                )
            else:
                return make_message(
                    content="New B", agent_id="agent_b", role="assistant"
                )
    ...
    @pytest.mark.asyncio
    async def test_low_convergence_continues(
        self, executor, mock_bus, mock_message_handler, mock_convergence_calculator
    ):
        ...
        # Mock messages
        msg_a = make_message(
            content="Different topic", agent_id="agent_a", role="assistant"
        )
        msg_b = make_message(
            content="Another topic", agent_id="agent_b", role="assistant"
        )
        mock_message_handler.get_agent_message.side_effect = [msg_a, msg_b]
    ...
    def test_set_custom_awareness(self, executor):
        ...
        custom_awareness = {"agent_a": mock_awareness_a, "agent_b": mock_awareness_b}
    ...
    def test_set_custom_awareness_partial(self, executor):
        ...
        custom_awareness = {"agent_a": mock_awareness_a, "agent_b": None}
    ...
    @pytest.mark.asyncio
    async def test_custom_awareness_injection_agent_a(
        self, executor, mock_bus, mock_message_handler
    ):
        ...
        executor.set_custom_awareness({"agent_a": mock_awareness_a, "agent_b": None})
    ...
    @pytest.mark.asyncio
    async def test_custom_awareness_injection_agent_b(
        self, executor, mock_bus, mock_message_handler
    ):
        ...
        executor.set_custom_awareness({"agent_a": None, "agent_b": mock_awareness_b})
    ...
    @pytest.mark.asyncio
    async def test_custom_awareness_injection_both_agents(
        self, executor, mock_bus, mock_message_handler
    ):
        ...
        executor.set_custom_awareness(
            {"agent_a": mock_awareness_a, "agent_b": mock_awareness_b}
        )
        ...
        assert (
            mock_bus.emit.call_count == 4
        )  # TurnStart + SystemPrompt_A + SystemPrompt_B + TurnComplete
    ...
    @pytest.mark.asyncio
    async def test_custom_awareness_no_prompts(
        self, executor, mock_bus, mock_message_handler
    ):
    ...
    @pytest.mark.asyncio
    async def test_custom_awareness_none_agents(
        self, executor, mock_bus, mock_message_handler
    ):
