...
class TestContextManagerEvents:
    ...
    def sample_messages(self):
        ...
        for i in range(50):
            messages.append(Message(
                ...
                content=f"This is message {i} with some content to make it longer. " * 10,
    ...
    @pytest.mark.asyncio
    async def test_event_emitted_on_truncation(self, context_manager, mock_event_bus, sample_messages):
        """Test that ContextTruncationEvent is emitted when truncation occurs."""
        # Prepare context with a low limit to force truncation
        with patch.object(ProviderContextManager, 'CONTEXT_LIMITS', {'anthropic': 1000}):
            result = context_manager.prepare_context(
                sample_messages,
                provider="anthropic",
                model="claude-3-haiku",
    ...
    @pytest.mark.asyncio
    async def test_event_contains_correct_data(self, context_manager, mock_event_bus, sample_messages):
    ...
    @pytest.mark.asyncio
    async def test_event_emission_failure_doesnt_break_truncation(self, context_manager, mock_event_bus, sample_messages):
