...
class TestEventDeserializer:
    ...
    def test_event_types_mapping(self):
        ...
        assert EventDeserializer.EVENT_TYPES["ConversationCreated"] == ConversationStartEvent
    ...
    def test_deserialize_event_exception_handling(self):
        ...
        with patch('pidgin.io.event_deserializer.logger') as mock_logger:
            with patch.object(EventDeserializer, '_build_conversation_start', side_effect=Exception("Test error")):
                result = EventDeserializer.deserialize_event(event_data)
                
                assert result is None
                mock_logger.error.assert_called_with("Failed to deserialize ConversationStartEvent: Test error")
...
class TestJsonlReading:
    ...
    def test_read_jsonl_events_with_empty_lines(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.jsonl', delete=False) as f:
            f.write('{"event_type": "TurnStartEvent", "conversation_id": "conv_1", "turn_number": 1}\n')
            f.write('\n')  # Empty line
            f.write('   \n')  # Whitespace only line
            f.write('{"event_type": "TurnStartEvent", "conversation_id": "conv_1", "turn_number": 2}\n')
            f.flush()
...
class TestIntegrationScenarios:
    ...
    def test_deserialize_all_event_types(self):
        ...
        event_samples = [
            ...
            {"event_type": "TurnCompleteEvent", "conversation_id": "conv_1", "turn_number": 1},
            {"event_type": "MessageRequestEvent", "conversation_id": "conv_1", "agent_id": "agent_a"},
            {"event_type": "MessageChunkEvent", "conversation_id": "conv_1", "agent_id": "agent_a"},
            {"event_type": "MessageCompleteEvent", "conversation_id": "conv_1", "agent_id": "agent_a"},
            {"event_type": "SystemPromptEvent", "conversation_id": "conv_1", "agent_id": "agent_a"},
            {"event_type": "ErrorEvent", "conversation_id": "conv_1"},
            {"event_type": "APIErrorEvent", "conversation_id": "conv_1", "agent_id": "agent_a"},
            {"event_type": "ProviderTimeoutEvent", "conversation_id": "conv_1", "agent_id": "agent_a"},
            ...
            {"event_type": "ContextTruncationEvent", "conversation_id": "conv_1", "agent_id": "agent_a"},
        ]
