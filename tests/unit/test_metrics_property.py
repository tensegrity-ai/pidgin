...
class TestLinguisticAnalyzerProperties:
    ...
    @given(
        st.dictionaries(st.text(min_size=1), st.integers(min_value=1, max_value=100))
    )
    ...
    def test_lexical_diversity_index_non_negative(self, word_count, vocab_size):
        ...
        ldi = LinguisticAnalyzer.calculate_lexical_diversity_index(
...
class TestConvergenceCalculatorProperties:
    ...
    def test_vocabulary_overlap_symmetry(self, vocab_a, vocab_b):
        ...
        overlap_ab = ConvergenceCalculator.calculate_vocabulary_overlap(
            vocab_a, vocab_b
        )
        overlap_ba = ConvergenceCalculator.calculate_vocabulary_overlap(
            vocab_b, vocab_a
        )
    ...
    def test_cross_repetition_range(self, message_a, message_b):
        ...
        repetition = ConvergenceCalculator.calculate_cross_repetition(
            message_a, message_b
        )
        assert 0.0 <= repetition <= 1.0
    ...
    def test_structural_similarity_range(self, message_a, message_b):
        ...
        similarity = ConvergenceCalculator.calculate_structural_similarity(
            message_a, message_b
        )
    ...
    def test_compression_ratio_range(self, text):
        ...
        assert (
            0.0 < ratio <= 2.0
        )  # Compression can sometimes be > 1 for very short texts
...
class TestMetricsCalculatorProperties:
    ...
    def test_metrics_bounded(self, turn_number, msg_a, msg_b):
        ...
        for metric in convergence_bounded:
            if metric in metrics["convergence"]:
                ...
                value = metrics["convergence"][metric]
                assert (
                    0.0 <= value <= 1.0 + 1e-10
                ), f"Metric {metric} value {value} is out of bounds"
    ...
    @given(
        st.lists(
            st.tuples(message_strategy(), message_strategy()), min_size=1, max_size=5
        )
    )
...
class TestEdgeCases:
    ...
    def test_punctuation_only_handling(self, punct_text):
        ...
        assert all(
            token == "" or all(c in string.punctuation for c in token)
            for token in tokens
        )
