...
from pidgin.providers.silent import SILENT_MODELS, SilentProvider
...
class TestSilentProvider:
    ...
    async def test_stream_response_ignores_messages(self):
        ...
        complex_messages = [
            Message(
                role="user",
                content="Very long message with lots of content",
                agent_id="agent_a",
            ),
            Message(
                role="assistant", content="Another long response", agent_id="agent_b"
            ),
...
class TestSilentProviderIntegration:
    ...
    async def test_meditation_mode_simulation(self):
        ...
        conversation = [
            ...
            Message(
                role="assistant",
                content="I think it's interesting because...",
                agent_id="agent_b",
            ),
    ...
    async def test_provider_consistency(self):
        ...
        test_scenarios = [
            ...
            [
                Message(role="user", content="", agent_id="agent_a"),
                Message(role="assistant", content="", agent_id="agent_b"),
            ],  # Empty content
            [
                Message(role="user", content="A" * 1000, agent_id="agent_a")
            ],  # Very long message
        ]
