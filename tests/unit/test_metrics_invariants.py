...
def repeated_word_text(draw):
    ...
    base_word = draw(st.text(alphabet='abcdefghijklmnopqrstuvwxyz', min_size=3, max_size=10))
...
class TestMetricInvariants:
    ...
    def test_unique_word_ratio_calculation(self, text):
        ...
        if agent_metrics['word_count'] > 0:
            expected_ratio = agent_metrics['vocabulary_size'] / agent_metrics['word_count']
    ...
    def test_convergence_symmetry_properties(self, msg_a, msg_b):
        ...
        assert abs(convergence_ab['vocabulary_overlap'] - convergence_ba['vocabulary_overlap']) < 1e-10
        
        # Length ratio should be symmetric
        assert abs(convergence_ab['length_ratio'] - convergence_ba['length_ratio']) < 1e-10
        
        # Cross repetition should be symmetric
        assert abs(convergence_ab['cross_repetition'] - convergence_ba['cross_repetition']) < 1e-10
...
class TestMetricRelationships:
    ...
    def test_convergence_score_components(self, messages):
        ...
        for i in range(0, len(messages) - 1, 2):
            metrics = calculator.calculate_turn_metrics(i // 2, messages[i], messages[i + 1])
    ...
    def test_turn_repetition_calculation(self, conversation):
        ...
        for turn_num, (msg_a, msg_b) in enumerate(conversation):
            ...
            if turn_num > 0:
                ...
                if words_a:
                    expected_a = len(words_a & all_agent_words['agent_a']) / len(words_a)
                else:
                    expected_a = 0.0
                    
                if words_b:
                    expected_b = len(words_b & all_agent_words['agent_b']) / len(words_b)
                else:
                    expected_b = 0.0
