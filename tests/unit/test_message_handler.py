...
class TestMessageHandler:
    ...
    def test_initialization(
        self, handler, mock_bus, mock_rate_limiter, mock_name_coordinator, mock_console
    ):
    ...
    @pytest.mark.asyncio
    async def test_record_request_completion(
        self, handler, mock_rate_limiter, mock_name_coordinator
    ):
        ...
        with patch("time.time", return_value=1001.5):  # End time
            await handler._record_request_completion(
                "test_model", message, 1000.0, history
            )
    ...
    async def test_get_agent_message_timeout(self, handler, mock_interrupt_handler):
        ...
        timeout_events = [
            call[0][0]
            for call in handler.bus.emit.call_args_list
            if isinstance(call[0][0], ProviderTimeoutEvent)
        ]
        assert len(timeout_events) == 1

    @pytest.mark.asyncio
    async def test_wait_for_message_with_interrupt_no_interrupt(
        self, handler, mock_interrupt_handler
    ):
