...
class TestDirectRouter:
    ...
    async def test_get_next_response_stream(self, router):
        ...
        chunks = []
        async for chunk, agent_id in router.get_next_response_stream(history, "agent_b"):
            chunks.append(chunk)
    ...
    async def test_streaming_empty_response(self, router):
        ...
        chunks = []
        async for chunk, agent_id in router.get_next_response_stream(history, "agent_a"):
            chunks.append(chunk)
    ...
    def test_build_agent_history_preserves_agent_ids(self, router):
        ...
        assert all(msg.agent_id == orig.agent_id 
                  for msg, orig in zip(agent_history, history))
