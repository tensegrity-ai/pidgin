...
class TestConductorIntegrationStyle:
    ...
    async def test_conversation_with_interrupt_integration(self, conductor_with_real_components):
        ...
        # Track events
        events_received = []
        conductor.bus.subscribe(ConversationEndEvent, lambda e: events_received.append(e))
        
        # Simulate interrupt after first turn
        async def simulate_interrupt():
            await asyncio.sleep(0.2)  # Let conversation start
    ...
    @pytest.mark.asyncio
    async def test_conversation_with_name_choosing_integration(self, conductor_with_real_components):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_integration(self, conductor_with_real_components, monkeypatch):
...
class TestConductorMockComparison:
    ...
    async def test_heavily_mocked_approach(self):
        ...
        output_manager.create_conversation_dir.return_value = ("test_conv_123", Path("/tmp/test"))
...
class TestWithHarness:
    ...
    async def test_simple_conversation_with_harness(self, tmp_path):
        ...
        try:
            ...
            assert end_events[0].end_reason == "max_turns"
