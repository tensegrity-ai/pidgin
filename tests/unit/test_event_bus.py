...
class TestEventBusSerializationEdgeCases:
    ...
    async def test_jsonl_serialization_error(self, event_bus, mock_pidgin_output_dir):
        ...
        with patch('json.dumps', side_effect=TypeError("Object not serializable")):
            with patch('pidgin.core.event_bus.logger') as mock_logger:
                ...
                assert any("Failed to serialize event" in str(call) for call in mock_logger.error.call_args_list)
    ...
    async def test_jsonl_write_error(self, event_bus, mock_pidgin_output_dir):
        ...
        with patch.object(event_bus, '_get_jsonl_file') as mock_get_file:
            ...
            with patch('pidgin.core.event_bus.logger') as mock_logger:
                ...
                assert any("Error writing to JSONL" in str(call) for call in mock_logger.error.call_args_list)
    ...
    async def test_close_error_handling(self, event_bus, mock_pidgin_output_dir):
        ...
        with patch('pidgin.core.event_bus.logger') as mock_logger:
            await event_bus.stop()
            # Should log error
            assert any("Error closing JSONL file" in str(call) for call in mock_logger.error.call_args_list)
    ...
    @pytest.mark.asyncio
    async def test_close_conversation_log_error(self, event_bus, mock_pidgin_output_dir):
    ...
    async def test_close_nonexistent_conversation(self, event_bus):
        ...
        # Should not crash when closing non-existent conversation
    ...
    async def test_model_field_handling(self, event_bus, mock_pidgin_output_dir):
        ...
        with patch('pidgin.core.event_bus.logger') as mock_logger:
            ...
            # Should log warning
            assert any("Unexpected type for model field" in str(call) for call in mock_logger.warning.call_args_list)
    ...
    @pytest.mark.asyncio
    async def test_jsonl_serialization_field_error(self, event_bus, mock_pidgin_output_dir):
