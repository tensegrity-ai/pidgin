...
from pidgin.core.events import Event, ConversationStartEvent, MessageCompleteEvent
from tests.builders import make_conversation_start_event, make_message_complete_event, make_message
...
class TestEventBusThreadSafety:
    ...
    async def test_concurrent_emit_operations(self, event_bus):
        ...
        assert len(history) == min(num_threads * events_per_thread, event_bus.max_history_size)
    ...
    async def test_stress_test_all_operations(self, event_bus):
        ...
        async def stress_operations(worker_id):
            ...
            for _ in range(100):
                op = random.choice(['emit', 'subscribe', 'unsubscribe', 
