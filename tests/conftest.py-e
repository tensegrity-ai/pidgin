...
def mock_config(monkeypatch, mock_config_dir):
    ...
    # Reset to defaults
    config._config = {
        "models": {"default_model_a": "local:test", "default_model_b": "local:test"},
        "display": {"show_progress": True, "show_cost": True},
        "experiments": {"max_parallel": 1, "default_repetitions": 3},
    }
...
def make_experiment():
    ...
    def _make(exp_id="test_exp", num_conversations=5):
        return Experiment(
            ...
            config={"model_a": "local:test", "model_b": "local:test", "max_turns": 10},
        ...
        for i in range(20):
            ...
            if pattern_type == "convergent" and i > 10:
                ...
                content = (
                    " ".join(words)
                    + " "
                    + fake.sentence(nb_words=max(1, length // 10 - 5))
                )
            else:
...
pytest.mark.integration = pytest.mark.skipif(
    "not config.getoption('--integration')", reason="need --integration option to run"
)
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "database: marks tests that require a real database connection"
    )
