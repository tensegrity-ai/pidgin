...
class TestConversationState:
    ...
    def test_add_message(self):
        ...
        # Add another pair
        state.add_message(
            Message(role="user", content="How are you?", agent_id="agent_a")
        )
        state.add_message(
            Message(role="assistant", content="I'm well", agent_id="agent_b")
        )
        assert state.turn_count == 2

    def test_serialization(self):
        """Test conversion to/from dictionary."""
        state = ConversationState(model_a="claude", model_b="gpt-4", max_turns=5)
    ...
    def test_checkpoint_save_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create state
            state = ConversationState(
                model_a="claude", model_b="gpt-4", transcript_path=str(transcript_path)
            )
            state.add_message(Message(role="user", content="Hello", agent_id="agent_a"))
            state.add_message(
    ...
    def test_resume_info(self):
        ...
        state = ConversationState(
            model_a="claude", model_b="gpt-4", max_turns=10, turn_count=3
        )
