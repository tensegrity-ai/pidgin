...
def create_github_issue(changes: Dict[str, any], sdk_updates: Dict[str, Tuple[str, str]]):
    """Create a GitHub issue for model changes."""
    
    if not changes["missing_known"] and not changes["configured_unknown"] and not sdk_updates:
    ...
    if changes["missing_known"]:
        lines.append("1. Review known models not in configuration for potential inclusion")
    if changes["configured_unknown"]:
        lines.append("2. Verify configured models are still available or update known list")
    if sdk_updates:
        lines.append("3. Update SDK dependencies and test compatibility")
    
    lines.append("")
    lines.append("Note: The 'known models' list is manually maintained and may need updating.")
