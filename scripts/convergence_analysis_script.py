...
class ConvergenceWeightAnalyzer:
    ...
    def analyze_conversation(
        self, jsonl_path: Path, thresholds: List[float] = None
    ) -> ConvergenceAnalysis:
    ...
    def _analyze_with_weights(
        self, messages: List[Any], weights: Dict[str, float], thresholds: List[float]
    ) -> Dict[str, Any]:
    ...
    def compare_profiles(
        self, analysis: ConvergenceAnalysis, threshold: float = 0.75
    ) -> pd.DataFrame:
    ...
    def analyze_batch(
        self, conversation_paths: List[Path], threshold: float = 0.75
    ) -> pd.DataFrame:
        ...
        for path in conversation_paths:
            try:
                ...
                for profile_name, profile_results in analysis.profiles.items():
                    ...
                    would_halt = (
                        halt_turn != "never" and halt_turn <= analysis.total_turns
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze convergence weights on existing conversations"
    )

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--conversation-path", type=Path, help="Path to single conversation directory"
    )
    group.add_argument(
        "--experiment-path",
        type=Path,
    ...
    group.add_argument("--jsonl", type=Path, help="Path to specific events.jsonl file")
    ...
    parser.add_argument(
        "--plot", action="store_true", help="Show plots of convergence over time"
    )
