...
class DisplayUtils:
    ...
    def _calculate_panel_width(self, content: Union[str, Text], title: str = "", 
                              min_width: int = 40, max_width: int = 80) -> int:
    ...
    def error(self, message: str, title: Optional[str] = None, 
              context: Optional[str] = None, use_panel: bool = True) -> None:
        ...
        if use_panel:
            ...
            # Plain text mode
            self.console.print(f"[{NORD_COLORS['nord11']}][FAIL] {message}[/{NORD_COLORS['nord11']}]")
            if context:
                self.console.print(f"[{NORD_COLORS['nord3']}]       {context}[/{NORD_COLORS['nord3']}]")
    ...
    def warning(self, message: str, title: Optional[str] = None,
        ...
        if use_panel:
            ...
            # Plain text mode
            self.console.print(f"[{NORD_COLORS['nord13']}]⚠ {message}[/{NORD_COLORS['nord13']}]")
            if context:
                self.console.print(f"[{NORD_COLORS['nord3']}]  {context}[/{NORD_COLORS['nord3']}]")
    
    def info(self, message: str, title: Optional[str] = None,
             use_panel: bool = True) -> None:
        ...
        if use_panel:
            ...
            # Plain text mode
            self.console.print(f"[{NORD_COLORS['nord7']}]◆ {message}[/{NORD_COLORS['nord7']}]")
    ...
    def success(self, message: str, use_panel: bool = False) -> None:
        ...
        if use_panel:
            ...
            self.console.print(f"[{NORD_COLORS['nord14']}][OK] {message}[/{NORD_COLORS['nord14']}]")
    
    def api_error(self, agent_name: str, provider: str, error_message: str,
                  retryable: bool = False, billing_url: Optional[str] = None) -> None:
        ...
        is_billing = any(phrase in error_lower for phrase in 
    ...
    def dim(self, message: str) -> None:
        ...
        """
        self.console.print(f"[{NORD_COLORS['nord3']}]{message}[/{NORD_COLORS['nord3']}]")
