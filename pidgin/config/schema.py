...
from ..constants import (
    ConvergenceProfiles, ConvergenceComponents, ConvergenceActions,
    DEFAULT_CONVERGENCE_WEIGHTS, DEFAULT_CONVERGENCE_THRESHOLD,
    DEFAULT_CONVERGENCE_PROFILE, DEFAULT_CONVERGENCE_ACTION
)
...
class ConvergenceWeights(BaseModel):
    ...
    def validate_sum(self) -> 'ConvergenceWeights':
        ...
        total = self.content + self.structure + self.sentences + self.length + self.punctuation
...
class ConvergenceConfig(BaseModel):
    ...
    profile: str = Field(
        default=DEFAULT_CONVERGENCE_PROFILE,
        description="Active convergence profile"
    )
    custom_weights: Optional[ConvergenceWeights] = Field(
        default=None,
        description="Custom weights when profile is 'custom'"
    )
...
class ContextManagementConfig(BaseModel):
    ...
    show_usage: bool = Field(
        default=True,
        description="Display context usage in UI"
    )
...
class DefaultsConfig(BaseModel):
    ...
    max_turns: int = Field(
        ...
        ge=1,
        le=1000,
        description="Default maximum turns per conversation"
    )
    manual_mode: bool = Field(
        default=False,
        description="Enable manual mode by default"
    )
...
class ExperimentProfileConfig(BaseModel):
    ...
    convergence_threshold: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Override convergence threshold"
    )
    convergence_action: Optional[Literal["stop", "warn"]] = Field(
        default=None,
        description="Override convergence action"
    )
...
class ProviderContextConfig(BaseModel):
    ...
    min_messages_retained: int = Field(
        default=10,
        ge=1,
        description="Never truncate below this many messages"
    )
    truncation_strategy: Literal["sliding_window"] = Field(
    ...
    safety_factor: float = Field(
        ...
        gt=0,
        le=1,
        description="Use this fraction of actual limits"
    )
