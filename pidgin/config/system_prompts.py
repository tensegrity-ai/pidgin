...
class CustomAwareness:
    ...
    def _validate_config(self):
        ...
        if not isinstance(prompts, dict):
            raise ValueError("'prompts' must be a dictionary mapping turn numbers to prompts")
        ...
        for turn, prompt_config in prompts.items():
            ...
            if invalid_keys:
                raise ValueError(f"Invalid keys in turn {turn}: {invalid_keys}. Valid keys: {valid_keys}")
    ...
    def get_initial_prompts(self, model_a_name=None, model_b_name=None) -> Dict[str, str]:
...
def get_system_prompts(awareness_a="basic", awareness_b="basic",
    ...
    Args:
        awareness_a: Awareness level for agent A (none, basic, firm, research) or path to YAML file
        awareness_b: Awareness level for agent B (none, basic, firm, research) or path to YAML file
    ...
    Returns:
        ...
        - custom_awareness_dict: Dict with agent_a and agent_b CustomAwareness objects (or None)
    """
