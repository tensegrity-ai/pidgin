...
class ContextWindowManager:
    ...
    def get_conversation_size(
        self, messages: Union[List[Message], List[Dict[str, Any]]], model: str
    ) -> int:
    ...
    def get_remaining_capacity(
        ...
        return {
            ...
            "percentage": min(100, (used / effective_limit) * 100)
            if effective_limit > 0
            else 100,
        }

    def predict_turns_remaining(
        self, messages: Union[List[Message], List[Dict[str, Any]]], model: str
    ) -> int:
        ...
        for i in range(max(0, len(messages) - 10), len(messages), 2):
            if i + 1 < len(messages):
                ...
                exchange_size = self.count_tokens(
                    str(content1), model
    ...
    def format_usage(self, capacity: Dict[str, Any]) -> str:
        ...
        return (
            f"{capacity['used']:,}/{capacity['limit']:,} tokens "
    ...
    def get_truncation_point(
        ...
        messages: Union[List[Message], List[Dict[str, Any]]],
        model: str,
        target_percentage: float = 50,
    ) -> int:
        """Find where to truncate messages to reach target capacity"""
        target_size = int(
            self.context_limits.get(model, 100000) * target_percentage / 100
        )
