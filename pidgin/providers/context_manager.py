...
class ProviderContextManager:
    ...
    def prepare_context(
        self, messages: List[Message], provider: str, model: Optional[str] = None
    ) -> List[Message]:
    ...
    def _truncate_sliding_window(
        self, messages: List[Message], token_limit: int, min_messages: int
    ) -> List[Message]:
    ...
    def get_context_usage(
        self, messages: List[Message], provider: str
    ) -> Tuple[int, int, float]:
    ...
    def _truncate_claude(
        self, messages: List[Message], model: Optional[str]
    ) -> List[Message]:
    ...
    def _truncate_default(
        self, messages: List[Message], provider: str, model: Optional[str]
    ) -> List[Message]:
    ...
    def _truncate_to_limit(
        ...
        logger.info(f"Env var truncation: {total_tokens} tokens > {token_limit} limit")

        return self._truncate_sliding_window(messages, token_limit, min_messages)

    def estimate_conversation_capacity(
        self, messages: List[Message], model_a: str, model_b: str
    ) -> Dict[str, Any]:
        """Estimate how many more turns possible before context limit.
        ...
        """
        # Get provider limits
        provider_a = self._get_provider_for_model(model_a)
        provider_b = self._get_provider_for_model(model_b)

        # Use smaller context window
        limit_a = self.context_limits.get(provider_a, 100000)
        limit_b = self.context_limits.get(provider_b, 100000)

        # Check for model-specific limits
        if model_a in self.MODEL_CONTEXT_LIMITS:
            limit_a = min(limit_a, self.MODEL_CONTEXT_LIMITS[model_a])
        if model_b in self.MODEL_CONTEXT_LIMITS:
            limit_b = min(limit_b, self.MODEL_CONTEXT_LIMITS[model_b])

        effective_limit = min(limit_a, limit_b)

        # Apply safety factor and reserve
        safety_factor = self.config.get("safety_factor", 0.9)
        reserve_ratio = self.config.get("context_reserve_ratio", 0.25)
        usable_limit = int(effective_limit * safety_factor * (1 - reserve_ratio))

        # Current usage
        current_tokens = self._estimate_tokens(messages)

        # Calculate growth rate from recent messages
        recent_growth = self._calculate_growth_rate(messages)

        # Predict turns remaining
        remaining_tokens = usable_limit - current_tokens
        turns_remaining = (
            int(remaining_tokens / recent_growth) if recent_growth > 0 else 999
        )
        ...
        return {
            ...
            "percentage_used": (current_tokens / usable_limit) * 100,
            "turns_remaining": max(0, turns_remaining),
            "model_limited_by": model_a if limit_a < limit_b else model_b,
            "growth_rate": recent_growth,
        }

    def _calculate_growth_rate(self, messages: List[Message]) -> float:
        """Calculate average token growth per turn from recent messages.
        ...
        """
        if len(messages) < 4:
            return 1000  # Default estimate for new conversations

        # Look at last 10 messages (5 turns)
        recent_messages = messages[-10:]
        recent_tokens = self._estimate_tokens(recent_messages)

        # Count turns (pairs of messages)
        turns = len(recent_messages) // 2

        return recent_tokens / turns if turns > 0 else 1000
