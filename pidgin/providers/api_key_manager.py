    ...
    def validate_required_providers(providers: List[str]) -> None:
        ...
        if missing:
            ...
            raise APIKeyError(
                ...
                + "\n".join(f"  • {provider.upper()}: Set {env_var}" 
                           for provider, env_var in [(p, APIKeyManager.PROVIDER_ENV_VARS.get(
                               p.lower(), f"{p.upper()}_API_KEY")) for p in missing]) + "\n\n"
                f"Export the required environment variables:\n"
                + "\n".join(f"  export {env_var}=your-{provider}-api-key" 
                           for provider, env_var in [(p, APIKeyManager.PROVIDER_ENV_VARS.get(
                               p.lower(), f"{p.upper()}_API_KEY")) for p in missing]) + "\n\n"
                f"For keychain integration:\n"
                f"  https://github.com/anthropics/pidgin#api-keys"
