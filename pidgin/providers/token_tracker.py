...
class GlobalTokenTracker:
    ...
    # Default rate limits (tokens per minute) - conservative estimates
    DEFAULT_RATE_LIMITS = {
        "anthropic": 400000,   # Tier 2 default
        "openai": 150000,      # Tier 2 default  
        "google": 360000,      # Standard tier
        "xai": 150000,         # Estimated
    }
    ...
    def _load_limits(self):
        ...
        for provider, settings in overrides.items():
            if "tokens_per_minute" in settings:
                self.rate_limits[provider] = settings["tokens_per_minute"]
    ...
    def can_make_request(
        ...
        rate_limit = self.rate_limits.get(provider_key, 150000)
        safe_limit = rate_limit * safety_margin
        ...
        wait_minutes = excess_tokens / rate_limit
    ...
    def get_usage_stats(self, provider: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "usage_percentage": (current_rate / rate_limit * 100) if rate_limit > 0 else 0,
