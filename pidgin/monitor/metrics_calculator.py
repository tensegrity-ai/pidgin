                        ...
                        for agent_id in ["agent_a", "agent_b"]:
                            ...
                            if model and (prompt_tokens > 0 or completion_tokens > 0):
                                ...
                                if config and config.input_cost_per_million is not None:
                                    cost = (
                                        (prompt_tokens / 1_000_000) * (config.input_cost_per_million or 0) +
                                        (completion_tokens / 1_000_000) * (config.output_cost_per_million or 0)
                                    )
                                    total_cost += cost
                                else:
                                    # Fallback pricing for models without config
                                    total_cost += ((prompt_tokens + completion_tokens) / 1_000_000) * 2.0
        ...
        if exp.conversations:
            ...
            for model in [model_a, model_b]:
                ...
                if config and config.input_cost_per_million is not None:
                    ...
                    cost = (
                        (input_tokens / 1_000_000) * (config.input_cost_per_million or 0) +
                        (output_tokens / 1_000_000) * (config.output_cost_per_million or 0)
                    )
                    total_cost += cost
                else:
