...
class ConversationState:
    ...
    def add_message(self, message: Message) -> None:
        ...
        if message.role in ["assistant", "user"]:
            self.turn_count = (
                len([m for m in self.messages if m.role in ["assistant", "user"]]) // 2
            )
    ...
    def save_checkpoint(self, transcript_path: Optional[Path] = None) -> Path:
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False, dir=checkpoint_path.parent, suffix=".tmp"
        ) as tmp:
    ...
    def list_checkpoints(self) -> List[Dict[str, Any]]:
        ...
        for cp_path in self.checkpoint_dir.glob("**/*.checkpoint"):
            try:
                ...
                checkpoints.append({"path": str(cp_path), "error": str(e)})
