...
class ImportService:
    ...
    def import_experiment_from_jsonl(self, exp_dir: Path) -> ImportResult:
        ...
        try:
            ...
            for jsonl_file in jsonl_files:
                turns_count = self._process_jsonl_file(jsonl_file, experiment_id, manifest)
    ...
    def _process_jsonl_file(self, jsonl_file: Path, experiment_id: str, manifest: Dict) -> int:
    ...
    def _prepare_turn_row(self, experiment_id: str, conversation_id: str, 
                         turn_number: int, turn_data: Dict, config: Dict,
                         metrics: Dict, messages: Dict) -> Dict[str, Any]:
    ...
    def _insert_turn(self, row: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to insert turn {row['conversation_id']}:{row['turn_number']}: {e}")
            raise
    ...
    def import_all_pending(self, experiments_dir: Path) -> List[ImportResult]:
        ...
        for exp_dir in experiments_dir.iterdir():
            ...
            if existing_count > 0:
                logger.info(f"Experiment {experiment_id} already imported ({existing_count} turns)")
                continue
