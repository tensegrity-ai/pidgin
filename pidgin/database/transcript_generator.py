...
class TranscriptGenerator:
    ...
    def generate_conversation_transcript(self, conversation_id: str) -> str:
        ...
        sections.append(self._format_summary_metrics(conv_data, token_data, len(turn_metrics)))
    ...
    def _format_header(self, conv_data: Dict, token_data: Dict) -> str:
        ...
        agent_a = conv_data.get("agent_a_chosen_name") or conv_data.get("agent_a_model", "Agent A")
        agent_b = conv_data.get("agent_b_chosen_name") or conv_data.get("agent_b_model", "Agent B")
    ...
    def _format_message_length_evolution(self, turn_metrics: List[Dict]) -> str:
        ...
        for start, end in ranges:
            metrics_in_range = [tm for tm in turn_metrics 
                              if start <= tm["turn_number"] <= end]
            
            if metrics_in_range:
                chars_a = [tm["message_a_length"] for tm in metrics_in_range 
                          if tm["message_a_length"] is not None]
                chars_b = [tm["message_b_length"] for tm in metrics_in_range 
                          if tm["message_b_length"] is not None]
                words_a = [tm["message_a_word_count"] for tm in metrics_in_range 
                          if tm["message_a_word_count"] is not None]
                words_b = [tm["message_b_word_count"] for tm in metrics_in_range 
                          if tm["message_b_word_count"] is not None]
                
                avg_chars_a.append(str(int(sum(chars_a) / len(chars_a))) if chars_a else "-")
                avg_chars_b.append(str(int(sum(chars_b) / len(chars_b))) if chars_b else "-")
                avg_words_a.append(str(int(sum(words_a) / len(words_a))) if words_a else "-")
                avg_words_b.append(str(int(sum(words_b) / len(words_b))) if words_b else "-")
            else:
                avg_chars_a.append("-")
                avg_chars_b.append("-")
                avg_words_a.append("-")
    ...
    def _format_vocabulary_metrics(self, turn_metrics: List[Dict]) -> str:
        ...
        for start, end in ranges:
            ...
            if metrics_in_range:
                ...
                lines.append(f"| {start}-{end} | {unique_a} | {unique_b} | {shared_count} | {overlap_pct:.1f}% |")
            else:
    ...
    def _format_response_times(self, turn_metrics: List[Dict]) -> str:
        ...
        for start, end in ranges:
            ...
            if metrics_in_range:
                times_a = [tm["message_a_response_time_ms"] for tm in metrics_in_range 
                          if tm.get("message_a_response_time_ms") is not None]
                times_b = [tm["message_b_response_time_ms"] for tm in metrics_in_range 
    ...
    def _format_token_usage(self, messages: List[Dict], token_data: Dict) -> str:
        ...
        for turn in sample_turns:
            ...
            if turn_messages:
                tokens_a = next((m["token_count"] for m in turn_messages 
                               if m["agent_id"] == "agent_a"), 0) or 0
                tokens_b = next((m["token_count"] for m in turn_messages 
                               if m["agent_id"] == "agent_b"), 0) or 0
                
                # Calculate cumulative up to this turn
                cumulative = sum(m["token_count"] or 0 for m in messages 
                               if m["turn_number"] <= turn)
                
                # Estimate cost (simplified)
                cost = cumulative * (token_data["total_cost_cents"] / token_data["total_tokens"]) / 100 if token_data["total_tokens"] > 0 else 0
                
                lines.append(f"| {turn} | {tokens_a} | {tokens_b} | {cumulative:,} | ${cost:.3f} |")
            else:
    ...
    def _format_transcript(self, messages: List[Dict], conv_data: Dict) -> str:
        ...
        # Get agent names
        agent_a_name = conv_data.get("agent_a_chosen_name") or conv_data.get("agent_a_model", "Agent A")
        agent_b_name = conv_data.get("agent_b_chosen_name") or conv_data.get("agent_b_model", "Agent B")
    ...
    def _generate_experiment_summary(self, exp_data: Dict) -> str:
        ...
        if exp_data.get('config'):
            config = json.loads(exp_data['config']) if isinstance(exp_data['config'], str) else exp_data['config']
