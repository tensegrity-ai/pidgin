...
class ExperimentRepository(BaseRepository):
    ...
    def get_experiment(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        if result:
            exp_dict = self.row_to_dict(result)
            
            # Parse JSON fields
            exp_dict['config'] = self.parse_json_field(exp_dict.get('config'))
            exp_dict['metadata'] = self.parse_json_field(exp_dict.get('metadata'))
