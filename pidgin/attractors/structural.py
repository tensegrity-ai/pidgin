...
class StructuralAnalyzer:
    ...
    def _analyze_line(self, line: str, position: int) -> StructuralElement:
        ...
        if (
            position == 0
            and features["punctuation"] in "!?"
            and features["length"] < 50
        ):
        ...
        if any(
            ...
            for marker in ["p.s.", "ps:", "p.s:", "note:", "btw:"]
        ):
...
class StructuralPatternDetector:
    ...
    def detect_attractor(self, messages: List[str]) -> Optional[Dict]:
        ...
        if alt_pattern:
            pattern_info = PatternRegistry.describe_alternating_pattern(
                alt_pattern["pair"]
            )
            return {
                "type": pattern_info["name"],
    ...
    def _find_repeating_pattern(self, signatures: List[str]) -> Optional[Dict]:
        ...
        for i, sig in enumerate(signatures):
            if sig in pattern_counts:
                ...
                pattern_counts[sig] = {"signature": sig, "count": 1, "positions": [i]}
    ...
    def _find_alternating_pattern(self, signatures: List[str]) -> Optional[Dict]:
        ...
        for i in range(0, len(signatures) - 3, 2):
            ...
            next_pair = (
                (signatures[i + 2], signatures[i + 3])
                if i + 3 < len(signatures)
                else None
            )
        ...
        for pattern_data in pair_counts.values():
            if (
                pattern_data["count"] >= self.threshold - 1
            ):  # Slightly lower threshold for alternating
    ...
    def _describe_pattern(self, signature: str) -> str:
        ...
        elif elements[0] == "EXCITED_OPENING" and elements[-1] in [
            "QUESTION",
            "POSTSCRIPT",
        ]:
