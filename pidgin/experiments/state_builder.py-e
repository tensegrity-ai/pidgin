...
class StateBuilder:
    ...
    def list_experiments(
        self, base_dir: Path, status_filter: Optional[List[str]] = None
    ...
    def _build_from_manifest(
        self, exp_dir: Path, manifest_path: Path
                    ...
                    with open(jsonl_file, "r") as f:
                        ...
                        for line in reversed(lines):
                            ...
                            try:
                                ...
                                if (
                                    event.get("event_type") == "TurnCompleteEvent"
                                    and event.get("conversation_id") == conv_id
                                    and event.get("convergence_score") is not None
                                ):
    ...
    def _get_conversation_timestamps(
                    ...
                    with open(jsonl_file, "r") as f:
                        ...
                        for line in lines:
                            ...
                            try:
                                event = json.loads(line.strip())
                                if (
                                    event.get("event_type") == "ConversationStartEvent"
                                    and event.get("conversation_id") == conv_id
                                ):
                        ...
                        for line in reversed(lines):
                            ...
                            try:
                                event = json.loads(line.strip())
                                if (
                                    event.get("event_type") == "ConversationEndEvent"
                                    and event.get("conversation_id") == conv_id
                                ):
        ...
        return {"started_at": started_at, "completed_at": completed_at}
    ...
    def _get_truncation_info(self, exp_dir: Path, conv_id: str) -> Dict[str, Any]:
                    ...
                    with open(jsonl_file, "r") as f:
                        ...
                        for line in lines:
                            ...
                            try:
                                event = json.loads(line.strip())
                                if (
                                    event.get("event_type") == "ContextTruncationEvent"
                                    and event.get("conversation_id") == conv_id
                                ):
                                    truncation_count += 1
        ...
        return {"count": truncation_count, "last_turn": last_truncation_turn}
                ...
                for line in f:
                    ...
                    try:
                        ...
                        if event_type == "ConversationStartEvent":
                            ...
                            metadata["original_experiment_id"] = event.get(
                                "experiment_id"
                            )
                        ...
                        # Track system prompts
                            ...
                            if agent_id:
                                metadata["system_prompts"][agent_id] = event.get(
                                    "prompt"
                                )
