                    ...
                    try:
                        ...
                        if event_type == "ConversationStartEvent":
                            if timestamp_str := event.get("timestamp"):
                                ...
                                timestamps["started_at"] = parser.parse_timestamp(timestamp_str)
                        
                        elif event_type == "ConversationEndEvent":
                            if timestamp_str := event.get("timestamp"):
                                from .manifest_parser import ManifestParser
                                parser = ManifestParser()
                                timestamps["completed_at"] = parser.parse_timestamp(timestamp_str)
            ...
            with open(events_file, "r") as f:
                for line in f:
                    try:
                        ...
                        if event_type == "ConversationStartEvent":
                            state.agent_a_model = event_data.get("agent_a_model", "unknown")
                            state.agent_b_model = event_data.get("agent_b_model", "unknown")
