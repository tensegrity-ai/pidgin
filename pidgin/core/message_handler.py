...
from .constants import RateLimits, SystemDefaults, Colors
...
class MessageHandler:
    ...
    async def get_agent_message(
        ...
        await self._handle_rate_limiting(
            conversation_id, agent, conversation_history
        )
    ...
    async def _handle_rate_limiting(
        ...
        if wait_time > RateLimits.RATE_LIMIT_WAIT_THRESHOLD:
            await self._emit_rate_limit_event(
                conversation_id, provider, wait_time
            )
    ...
    async def _request_and_wait_for_message(
        ...
        try:
            message = await self._wait_for_message_with_interrupt(
                future, conversation_id, agent, turn_number,
                interrupt_handler, timeout
            )
            
            if message:
                await self._record_request_completion(
                    agent.model, message, request_start,
    ...
    async def _record_request_completion(
        ...
        actual_tokens = len(message.content) // RateLimits.TOKEN_CHAR_RATIO + payload_tokens
    ...
    def _estimate_payload_tokens(
        ...
        estimated = int(total_chars / RateLimits.TOKEN_CHAR_RATIO * RateLimits.TOKEN_OVERHEAD_MULTIPLIER)
