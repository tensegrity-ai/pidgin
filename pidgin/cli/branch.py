...
from .helpers import validate_model_id, parse_temperature
from .constants import DEFAULT_TURNS, DEFAULT_TEMPERATURE
...
@click.option('--turn', '-t', type=int,
              help='Turn number to branch from (default: last turn)')
@click.option('--agent-a', '-a',
              help='Override agent A model')
@click.option('--agent-b', '-b',
              help='Override agent B model')
@click.option('--temperature', type=float,
              help='Override temperature for both agents')
@click.option('--temp-a', type=float,
              help='Override temperature for agent A')
@click.option('--temp-b', type=float,
              help='Override temperature for agent B')
@click.option('--awareness', '-w',
              help='Override awareness level or YAML file')
@click.option('--awareness-a',
              help='Override awareness for agent A')
@click.option('--awareness-b',
              help='Override awareness for agent B')
@click.option('--max-turns', type=int,
              help='Override maximum turns')
@click.option('--name', '-n',
              help='Name for the branched experiment')
@click.option('--repetitions', '-r', type=int, default=1,
              help='Number of branches to create (default: 1)')
@click.option('--quiet', '-q', is_flag=True,
              help='Run in background')
@click.option('--spec', '-s', type=click.Path(),
              help='Save branch configuration as YAML spec')
def branch(conversation_id: str, turn: Optional[int], agent_a: Optional[str],
           agent_b: Optional[str], temperature: Optional[float],
           temp_a: Optional[float], temp_b: Optional[float],
           awareness: Optional[str], awareness_a: Optional[str],
           awareness_b: Optional[str], max_turns: Optional[int],
           name: Optional[str], repetitions: int, quiet: bool,
           spec: Optional[str]):
    ...
    if branch_config['agent_a_model'] != original_config['agent_a_model']:
        changes.append(f"Agent A: {original_config['agent_a_model']} → {branch_config['agent_a_model']}")
    if branch_config['agent_b_model'] != original_config['agent_b_model']:
        changes.append(f"Agent B: {original_config['agent_b_model']} → {branch_config['agent_b_model']}")
    if branch_config.get('temperature_a') != original_config.get('temperature_a'):
        changes.append(f"Temp A: {original_config.get('temperature_a', 'default')} → {branch_config.get('temperature_a')}")
    if branch_config.get('temperature_b') != original_config.get('temperature_b'):
        changes.append(f"Temp B: {original_config.get('temperature_b', 'default')} → {branch_config.get('temperature_b')}")
    ...
    try:
        ...
        if quiet:
            ...
            # Show live display
            console.print(f"[#4c566a]Ctrl+C to exit display • experiment continues[/#4c566a]")
            console.print()
            ...
            try:
                ...
                console.print()
                display.info("Display exited. Branch continues in background.", use_panel=False)
